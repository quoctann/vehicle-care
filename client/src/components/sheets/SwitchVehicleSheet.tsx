  ...
  return (
    <Sheet open={open} onOpenChange={(nextOpen) => !nextOpen && close()}>
      ...
      >
        ...
        <SheetTitle className="mx-0.5 mb-1 text-[15px] font-semibold tracking-[-0.01em]">Switch vehicle</SheetTitle>
        <SheetDescription className="sr-only">Choose a vehicle or add another one.</SheetDescription>

        <div className="mt-2 max-h-[50dvh] overflow-y-auto rounded-2xl border border-border-subtle bg-card shadow-[0_1px_1px_rgba(44,54,53,.025),inset_0_0_0_2px_#fff]">
          {vehicles.length === 0 ? (
            <p className="px-4 py-5 text-center text-sm text-muted-foreground">No vehicles yet</p>
          ) : (
